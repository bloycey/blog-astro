...
If you want a full explanation of how the above code works you can check out [the docs](https://docs.netlify.com/functions/overview/) but nothing too fancy is going on here.

First we destructure the data from the request, then we check if the honeypot has been triggered. If it has we return a 500 status code and don't hit the database. If it hasn't we send the data to the database and return a 200 status code.
